...
contract("LendingPool - stable rate economy tests", async ([deployer, ...users]) => {
  ...
  let _PTokenInstances: IPTokenInstances
  ...
  it("BORROW - Test user cannot borrow using the same currency as collateral", async () => {
    const {PDai: PDaiInstance} = _PTokenInstances
  ...
  it("BORROW - Test user cannot borrow more than 25% of the liquidity available", async () => {
    ...
    const amountPDaiToRedeem = await convertToCurrencyDecimals(PDaiInstance.address, "1000")
    await PDaiInstance.redeem(amountPDaiToRedeem, {
      from: _borrowerAddress,
    })
  ...
  it("BORROW - Test user can still borrow  a currency that he previously deposited as a collateral but he transferred/redeemed", async () => {
    ...
    //user transfers to another address all the overlying PDai

    const PDaiBalance = await PDaiInstance.balanceOf(user)

    await PDaiInstance.transfer(users[3], PDaiBalance, {
      from: user,
    })
    ...
    expect(userData.currentPTokenBalance.toString()).to.be.equal("0")
