...
contract('LendingPool liquidation - liquidator receiving PToken', async ([deployer, ...users]) => {
  ...
  it('LIQUIDATION - Deposits ETH, borrows DAI/Check liquidation fails because health factor is above 1', async () => {
    ...
    const aEthInstance: PTokenInstance = await getTruffleContractInstance(
      artifacts,
      ContractId.PToken,
      await _lendingPoolCoreInstance.getReservePTokenAddress(ETHEREUM_ADDRESS)
    );
